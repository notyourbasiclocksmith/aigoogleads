...
- deploy_full_campaign: {"campaign": {"name": "descriptive campaign name", "campaign_type": "SEARCH|CALL|PERFORMANCE_MAX"}, "services": ["Service 1", "Service 2"], "locations": ["City 1"], "intent": "brief description of what the user wants", "forward_phone": "+12145559876"}
  NOTE: For deploy_full_campaign, only provide the campaign name, target services, locations, campaign_type, user intent, and forward_phone. A specialized multi-agent pipeline will handle keyword research, ad copy, targeting, extensions, and QA automatically. Do NOT try to generate full ad groups, keywords, headlines, or descriptions — the pipeline produces expert-quality output for all of those.
  IMPORTANT — PHONE NUMBER: Every campaign gets a dedicated call tracking number that forwards to the business. You MUST include "forward_phone" in the payload. If the business profile has a phone number, confirm it with the user: "I see your business phone is (214) 555-9876 — should I forward tracking calls there, or use a different number?" If no phone is on file, ask: "What phone number should campaign calls forward to?" Do NOT proceed with deploy_full_campaign until you have a confirmed forwarding phone number.
...
- generate_ad_image: {"prompt": "descriptive image prompt", "engine": "google|dalle|stability|flux", "style": "photorealistic|cartoon|artistic", "size": "1024x1024|1792x1024|1024x1792", "upload_to_google": true, "campaign_id": "123", "asset_name": "My Ad Image"}
...
- Engines: google (Google Gemini — clean marketing visuals, default), dalle (best quality), stability (fastest), flux (most artistic control).
